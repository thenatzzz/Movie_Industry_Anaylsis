...
def main(inputs):
    ...
    company_indexer = StringIndexer(inputCol="production_companies", outputCol="production_companies_index", handleInvalid = "skip")
    country_indexer = StringIndexer(inputCol="production_countries", outputCol="production_countries_index",  handleInvalid = "skip")
    ...
    assembler = VectorAssembler(inputCols = ["production_companies_index", "production_countries_index", "genres_index", "release_date", "budget", "avg_rating", "runtime", "cast_index", "director_index"], outputCol = 'features')
    ...
    second_assembler = VectorAssembler(inputCols = ["production_companies_index", "production_countries_index", "genres_index", "release_date", "avg_rating", "runtime", "cast_index", "director_index"], outputCol = 'features')
    ...
    third_assembler = VectorAssembler(
        inputCols=["production_companies_index", "production_countries_index", "release_date",
                   "avg_rating", "runtime", "cast_index", "director_index"], outputCol='features')

    third_pipeline = Pipeline(
        stages=[company_indexer, country_indexer, cast_indexer, director_indexer, third_assembler,
                regressor])
    ...
    forth_assembler = VectorAssembler(
        inputCols=["production_companies_index", "production_countries_index", "release_date",
                   "avg_rating", "cast_index", "director_index"], outputCol='features')

    forth_pipeline = Pipeline(
        stages=[company_indexer, country_indexer, cast_indexer, director_indexer, forth_assembler,
                regressor])
